...
template <typename T, typename TagT, typename LabelT>
const T *Index<T, TagT, LabelT>::data_ptr_local(uint32_t local_id) const noexcept
{
    auto *mem = dynamic_cast<const InMemDataStore<T> *>(_data_store.get());
    if (!mem)
        return nullptr;
    return mem->row_ptr(local_id);
}

template <typename T, typename TagT, typename LabelT>
std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::iterate_to_fixed_point_emit(
    InMemQueryScratch<T> *scratch, uint32_t Lsize, const std::vector<uint32_t> &init_ids,
    std::vector<uint32_t> *out_ids, std::vector<float> *out_dists, size_t reserve_hint)
{
    if (!out_ids)
    {
        throw diskann::ANNException("out_ids must not be null", -1, __FUNCSIG__, __FILE__, __LINE__);
    }
    out_ids->clear();
    if (out_dists)
        out_dists->clear();
    if (reserve_hint)
    {
        out_ids->reserve(reserve_hint);
        if (out_dists)
            out_dists->reserve(reserve_hint);
    }

    auto &inserted_into_pool_bs = scratch->inserted_into_pool_bs();
    auto &inserted_into_pool_rs = scratch->inserted_into_pool_rs();
    ...
    // const bool fast_iterate = total_pts <= MAX_POINTS_FOR_USING_BITSET;
    const bool fast_iterate = false;

    if (fast_iterate)
    {
        if (inserted_into_pool_bs.size() < total_pts)
        {
            auto resize_sz = 2 * total_pts > MAX_POINTS_FOR_USING_BITSET ? MAX_POINTS_FOR_USING_BITSET : 2 * total_pts;
            inserted_into_pool_bs.resize(resize_sz);
        }
    }
    else
    {
    ...
    T *aligned_query = scratch->aligned_query();
    const uint32_t adim = static_cast<uint32_t>(_data_store->get_aligned_dim());
    auto *dist_fn = _data_store->get_dist_fn();
    const bool use_neg = (_dist_metric == diskann::Metric::INNER_PRODUCT);
    std::vector<T> tmp_vec(adim);

    auto compute_dists_fp = [&](const std::vector<uint32_t> &ids, std::vector<float> &dists_out) {
        dists_out.resize(ids.size());
        for (size_t i = 0; i < ids.size(); ++i)
        {
            _data_store->get_vector(ids[i], tmp_vec.data());
            float d = static_cast<float>(dist_fn->compare(aligned_query, tmp_vec.data(), adim));
            dists_out[i] = use_neg ? -d : d;
        }
    };
    ...
    for (uint32_t id : init_ids)
    {
        ...
        if (is_not_visited(id))
        {
            ...
            _data_store->get_vector(id, tmp_vec.data());
            float d = static_cast<float>(dist_fn->compare(aligned_query, tmp_vec.data(), adim));
            best_L_nodes.insert(Neighbor(id, use_neg ? -d : d));
        }
    }

    uint32_t hops = 0, cmps = 0;

    while (best_L_nodes.has_unexpanded_node())
    {
        ++hops;
        const Neighbor cur = best_L_nodes.closest_unexpanded();
        const uint32_t n = cur.id;

        out_ids->push_back(n);
        if (out_dists)
            out_dists->push_back(cur.distance);
        ...
        if (!id_scratch.empty())
        {
            compute_dists_fp(id_scratch, dist_scratch);
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::iterate_and_export_visited(const T *query, const uint32_t L,
                                                        std::vector<uint32_t> &out_ids, std::vector<float> *out_dists,
                                                        bool /*clear_after_copy*/)
{
    ScratchStoreManager<InMemQueryScratch<T>> manager(_query_scratch);
    auto scratch = manager.scratch_space();
    if (L > scratch->get_L())
    ...
    scratch->best_l_nodes().clear();
    scratch->best_l_nodes().reserve(L);
    scratch->inserted_into_pool_rs().clear();
    scratch->id_scratch().clear();
    scratch->dist_scratch().clear();

    const std::vector<uint32_t> init_ids = this->get_init_ids();
    _data_store->preprocess_query(query, scratch);

    size_t reserve_hint = static_cast<size_t>(L * 2);
    (void)this->iterate_to_fixed_point_emit(scratch, L, init_ids, &out_ids, out_dists, reserve_hint);
}

template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::copy_neighbors_local(uint32_t local_id, std::vector<uint32_t> &out) const
{
    auto nbrs = _graph_store->get_neighbours(local_id);
    out.assign(nbrs.begin(), nbrs.end());
}

template DISKANN_DLLEXPORT void Index<float, uint32_t, uint32_t>::iterate_and_export_visited(
    const float *, const uint32_t, std::vector<uint32_t> &, std::vector<float> *, bool);
