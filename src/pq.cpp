                                 ...
                                 bool use_opq)
{
    ...
    // Buffers: block_data_T (T*dim*B), block_data_float (float*dim*B), block_data_tmp (float*dim*B)
    size_t target_ram_bytes = 256 * 1024 * 1024; // 256 MB default
    if (const char* env_p = std::getenv("SIGMA_CHUNK_BYTES")) {
        try {
            target_ram_bytes = std::stoull(env_p);
        } catch (...) {}
    }
    ...
    if (calculated_block_size > 1000000) calculated_block_size = 1000000;
