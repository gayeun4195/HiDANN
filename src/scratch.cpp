...
template <typename T> SSDThreadData<T>::~SSDThreadData()
{
    if (refinement_buffer != nullptr)
    {
        diskann::aligned_free((void *)refinement_buffer);
        refinement_buffer = nullptr;
    }
}

template <typename T> void SSDThreadData<T>::resize_refinement_buffer(size_t new_size, size_t alignment)
{
    if (new_size > refinement_buffer_alloc_size)
    {
        if (refinement_buffer != nullptr)
        {
            diskann::aligned_free((void *)refinement_buffer);
        ...
        // Adaptive Growth: Use exact new_size or maybe add some slack?
        // User requested strict adherence, but for safety in "Adaptive" strategy,
        // we usually match requests.
        // Let's alloc exactly new_size to satisfy user strictness.

        diskann::alloc_aligned((void **)&refinement_buffer, new_size, alignment);
        refinement_buffer_alloc_size = new_size;
    }
}
