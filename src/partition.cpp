...
static size_t get_safe_block_size(size_t dim, size_t point_size_bytes) {
    size_t target_ram = 256 * 1024 * 1024; // default 256MB
    if (const char* env_p = std::getenv("SIGMA_CHUNK_BYTES")) {
        try {
    ...
    if(bs < 1024) bs = 1024;
    if(bs > 1000000) bs = 1000000;

    // Optional: Log once or always. For safety in tight loops, maybe just calc.
    // Callers print "Dynamic Block Size" usually.
    return bs;
}
                              ...
                              size_t graph_degree, const std::string prefix_path, size_t k_base)
{
    ...
    if (const char *s = std::getenv("K_BASE"))
    {
        if (*s)
        {
            errno = 0;
            ...
            if (errno == 0 && end && *end == '\0' && v > 0ULL)
            {
                k_base = static_cast<size_t>(v);
                diskann::cout << "[partition_with_ram_budget] k_base from env K_BASE=" << k_base << std::endl;
            }
            else
            {
                diskann::cout << "[partition_with_ram_budget] WARNING: invalid K_BASE='" << s
                              << "'; keep k_base=" << k_base << std::endl;
            }

            size_t odd = (k_base % 2 == 1) ? k_base : (k_base + 1);
            num_parts = static_cast<int>(odd > 0 ? odd : 1);
        }
    }
