                              ...
                              const uint32_t Lf)
{
    ...
    auto all_t0 = clock_t::now();
    double part_s = 0.0, build_s = 0.0, merge_s = 0.0;
    ...
    ram_budget = (full_index_ram * 0.2) / GiB;
    ...
    build_s = std::chrono::duration<double>(clock_t::now() - t_build0).count();
    diskann::cout << timer.elapsed_seconds_for_step("building indices on shards") << std::endl;

    timer.reset();
    auto t_merge0 = clock_t::now();
    ...
    merge_s = std::chrono::duration<double>(clock_t::now() - t_merge0).count();
    ...
    double total_s = std::chrono::duration<double>(clock_t::now() - all_t0).count();
    std::cout << "==== build_merged_vamana_index summary ====\n";
    std::cout << "mode=partition-merge\n";
    std::cout << "parts=" << num_parts << "\n";
    std::cout << "partitioning_s=" << part_s << "\n";
    std::cout << "build_shards_s=" << build_s << "\n";
    std::cout << "merge_s=" << merge_s << "\n";
                     ...
                     const uint32_t Lf)
{
    ...
    generate_quantized_data<T>(data_file_to_use, pq_pivots_path, pq_compressed_vectors_path, compareMetric, 0.05,
