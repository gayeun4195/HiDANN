...
template <typename T, typename TagT = uint32_t, typename LabelT = uint32_t> class Index : public AbstractIndex
{
  ...
  public:
    ...
    DISKANN_DLLEXPORT const T *data_ptr_local(uint32_t local_id) const noexcept;

    DISKANN_DLLEXPORT void iterate_and_export_visited(const T *query, const uint32_t L, std::vector<uint32_t> &out_ids,
                                                      std::vector<float> *out_dists = nullptr,
                                                      bool clear_after_copy = false);

    DISKANN_DLLEXPORT void copy_neighbors_local(uint32_t local_id, std::vector<uint32_t> &out) const;
  ...
  protected:
    ...
    std::pair<uint32_t, uint32_t> iterate_to_fixed_point_emit(InMemQueryScratch<T> *scratch, uint32_t Lsize,
                                                              const std::vector<uint32_t> &init_ids,
                                                              std::vector<uint32_t> *out_ids,
                                                              std::vector<float> *out_dists, size_t reserve_hint = 0);
