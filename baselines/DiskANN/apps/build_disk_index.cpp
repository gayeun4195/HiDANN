...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        optional_configs.add_options()("search_ram_budget_ratio", po::value<float>()->default_value(0.0f),
                                       "Ratio of total index size to allocate for search RAM budget (overrides B)");
        optional_configs.add_options()("pq_sampling_rate", po::value<float>()->default_value(0.0f),
                                       "Sampling rate for PQ training (overrides default 256000/N)");
        optional_configs.add_options()("detailed_stats", po::bool_switch()->default_value(false),
                                       "Print detailed timing breakdown for merged index build");
        ...
        optional_configs.add_options()("chunk_bytes", po::value<uint64_t>(&chunk_bytes)->default_value(0),
                                       "Dynamic Chunk Budget (bytes) for limited memory construction");
        ...
        if (vm["use_opq"].as<bool>())
            use_opq = true;
        if (vm.count("search_ram_budget_ratio"))
            search_ram_budget_ratio = vm["search_ram_budget_ratio"].as<float>();
        if (vm.count("pq_sampling_rate"))
            pq_sampling_rate = vm["pq_sampling_rate"].as<float>();
        if (vm["detailed_stats"].as<bool>())
            detailed_stats = true;
    }
    ...
    if (chunk_bytes > 0)
    {
        ...
        std::cout << "[build_disk_index] Applied DISKANN_CHUNK_BYTES=" << s << " from --chunk_bytes\n";
    }
    ...
    params = params + " " + std::string(std::to_string(search_ram_budget_ratio)) + " " +
             std::string(std::to_string(pq_sampling_rate)) + " " + std::string(std::to_string(detailed_stats));
