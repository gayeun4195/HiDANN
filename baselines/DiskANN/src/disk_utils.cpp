...
namespace
{
constexpr uint64_t SEQUENTIAL_READ_CACHE_DROP_INTERVAL = 4ull * 1024ull * 1024ull;
constexpr uint64_t SEQUENTIAL_WRITE_CACHE_DROP_INTERVAL = 64ull * 1024ull * 1024ull;

void drop_file_cache(const std::string &filename)
{
#if defined(__linux__)
    int fd = ::open(filename.c_str(), O_RDONLY);
    if (fd < 0)
    {
        diskann::cerr << "WARNING: failed to open " << filename
                      << " for cache drop: " << std::strerror(errno) << std::endl;
        return;
    }
    ...
    if (ret != 0)
    {
        diskann::cerr << "WARNING: posix_fadvise(DONTNEED) failed for " << filename << ": " << std::strerror(ret)
                      << std::endl;
    }
    ::close(fd);
#else
...
class SequentialReadCacheDropper
{
  public:
    SequentialReadCacheDropper(const std::string &filename, uint64_t interval_bytes)
        : filename(filename), interval_bytes(interval_bytes)
    {
#if defined(__linux__)
        ...
        {
            diskann::cerr << "WARNING: failed to open " << filename
                          << " for sequential read cache drop: " << std::strerror(errno) << std::endl;
            interval_bytes = 0;
        }
#else
        (void)filename;
...
#if defined(__linux__)
        logical_pos += bytes;
        if (fd < 0 || interval_bytes == 0 || logical_pos - last_drop_offset < interval_bytes)
            return;

        const uint64_t drop_len = logical_pos - last_drop_offset;
        int ret = ::posix_fadvise(fd, (off_t)last_drop_offset, (off_t)drop_len, POSIX_FADV_DONTNEED);
        if (ret != 0 && !warned)
        {
            diskann::cerr << "WARNING: posix_fadvise(DONTNEED) failed for " << filename << ": " << std::strerror(ret)
                          << std::endl;
                 ...
                 const std::string &labels_to_medoids_file)
{
    ...
    for (auto &idmap : idmaps)
    {
        ...
        total_occurrences += idmap.size();
    }
    nnodes++;
    diskann::cout << "# nodes: " << nnodes << ", max. degree: " << max_degree << std::endl;

    // Stream the node -> shard inverse map instead of materializing all
    // replicated (node, shard) pairs. Each idmap is written in increasing
    // global node id order, matching the row order in the shard index.
    struct ShardCursor
    {
        uint32_t node_id;
        uint32_t shard_id;
    ...
    struct ShardCursorGreater
    {
        bool operator()(const ShardCursor &left, const ShardCursor &right) const
        {
    ...
    std::priority_queue<ShardCursor, std::vector<ShardCursor>, ShardCursorGreater> node_shard_heap;
    for (size_t shard = 0; shard < nshards; shard++)
    {
        if (!idmaps[shard].empty())
        {
            node_shard_heap.push({idmaps[shard][0], (uint32_t)shard, 0});
        }
    }
    diskann::cout << "Initialized streaming node -> shards heap with " << node_shard_heap.size()
                  << " shard cursors for " << total_occurrences << " replicated assignments" << std::endl;
    ...
    for (size_t i = 0; i < nshards; i++)
    {
        ...
        vamana_readers[i].enable_periodic_cache_drop(SEQUENTIAL_READ_CACHE_DROP_INTERVAL);
    ...
    merged_vamana_writer.enable_periodic_cache_drop(SEQUENTIAL_WRITE_CACHE_DROP_INTERVAL);
    ...
    std::vector<uint32_t> shard_nhood(max_input_width);

    uint32_t nnbrs = 0, shard_nnbrs = 0;
    uint32_t cur_id = 0;
    bool have_cur_id = false;
    ...
    while (!node_shard_heap.empty())
    {
        ShardCursor cursor = node_shard_heap.top();
        node_shard_heap.pop();

        uint32_t node_id = cursor.node_id;
        uint32_t shard_id = cursor.shard_id;
        uint32_t local_id = cursor.local_id;
        uint32_t next_local_id = local_id + 1;
        if (next_local_id < idmaps[shard_id].size())
        {
            node_shard_heap.push({idmaps[shard_id][next_local_id], shard_id, next_local_id});
        }
        ...
        if (shard_nnbrs > shard_nhood.size())
        {
            shard_nhood.resize(shard_nnbrs);
        }
        if (shard_nnbrs > 0)
...
int build_merged_vamana_index(std::string base_file, diskann::Metric compareMetric, uint32_t L, uint32_t R,
                              ...
                              const uint32_t Lf, const float ram_budget_ratio, const bool use_detailed_stats)
{
    size_t base_num, base_dim;
    diskann::get_bin_metadata(base_file, base_num, base_dim);

    double full_index_ram = estimate_ram_usage(base_num, (uint32_t)base_dim, sizeof(T), R);
    if (ram_budget_ratio > 0)
    {
        const double ratio_ram_budget = full_index_ram * ram_budget_ratio / (1024 * 1024 * 1024);
        if (ram_budget <= 0)
        {
            ram_budget = ratio_ram_budget;
            diskann::cout << "Using ratio-derived RAM budget " << ram_budget << "GB (" << ram_budget_ratio * 100
                          << "% of full index size)" << std::endl;
        }
        else
        {
            const double explicit_ram_budget = ram_budget;
            if (ratio_ram_budget < ram_budget)
            {
                ram_budget = ratio_ram_budget;
            }
            diskann::cout << "Ratio-derived RAM budget is " << ratio_ram_budget << "GB (" << ram_budget_ratio * 100
                          << "% of full index size); explicit indexing RAM budget is " << explicit_ram_budget
                          << "GB; using effective RAM budget " << ram_budget << "GB" << std::endl;
        }
    }
    ...
    if (num_parts <= 0)
    {
        diskann::cerr << "Error: partition_with_ram_budget failed." << std::endl;
        return -1;
    }
                        ...
                        const std::string reorder_data_file)
{
    ...
    base_reader.enable_periodic_cache_drop(SEQUENTIAL_READ_CACHE_DROP_INTERVAL);
    ...
    diskann_writer.enable_periodic_cache_drop(SEQUENTIAL_WRITE_CACHE_DROP_INTERVAL);
    SequentialReadCacheDropper vamana_cache_dropper(mem_index_file, SEQUENTIAL_READ_CACHE_DROP_INTERVAL);
    auto read_vamana = [&](char *buf, uint64_t nbytes) {
        vamana_reader.read(buf, nbytes);
        vamana_cache_dropper.advance(nbytes);
    };
    auto skip_vamana = [&](uint64_t nbytes) {
        vamana_reader.seekg((std::streamoff)nbytes, vamana_reader.cur);
        vamana_cache_dropper.advance(nbytes);
    };
    ...
    read_vamana((char *)&index_file_size, sizeof(uint64_t));
    ...
    read_vamana((char *)&width_u32, sizeof(uint32_t));
    read_vamana((char *)&medoid_u32, sizeof(uint32_t));
    read_vamana((char *)&vamana_frozen_num, sizeof(uint64_t));
    // compute
    ...
    { // Write multiple nodes per sector
        for (uint64_t sector = 0; sector < n_sectors; sector++)
        {
                 ...
                 sector_node_id++)
            {
                ...
                read_vamana((char *)&nnbrs, sizeof(uint32_t));
                ...
                // read node's nhood
                read_vamana((char *)nhood_buf, (std::min)(nnbrs, width_u32) * sizeof(uint32_t));
                if (nnbrs > width_u32)
                {
                    skip_vamana((nnbrs - width_u32) * sizeof(uint32_t));
                }
                     ...
                     const uint32_t Lf)
{
    ...
    float search_ram_budget_ratio = 0.0f;
    if (param_list.size() >= 10)
    {
        search_ram_budget_ratio = (float)atof(param_list[9].c_str());
    }
    ...
    {
        float pq_sampling_rate = (float)atof(param_list[10].c_str());
        if (pq_sampling_rate > 0)
        {
            p_val = pq_sampling_rate;
            diskann::cout << "Using fixed PQ sampling rate: " << p_val << std::endl;
        }
    }
    ...
    {
        use_detailed_stats = (bool)atof(param_list[11].c_str());
    }
    ...
    float ram_budget_ratio = 0.0f;
    if (param_list.size() >= 10)
    {
        ram_budget_ratio = (float)atof(param_list[9].c_str());
    }
...
// This code should only be available in the "build" DLL.
    ...
    const int merged_vamana_ret =
        diskann::build_merged_vamana_index<T, LabelT>(data_file_to_use.c_str(), diskann::Metric::L2, L, R, p_val,
                                                      ...
                                                      ram_budget_ratio, use_detailed_stats);
    if (merged_vamana_ret != 0)
    {
        diskann::cerr << "Error: building merged vamana index failed." << std::endl;
        return merged_vamana_ret;
    }
...
template DISKANN_DLLEXPORT int build_merged_vamana_index<int8_t, uint32_t>(
    ...
    const float ram_budget_ratio, const bool use_detailed_stats);
