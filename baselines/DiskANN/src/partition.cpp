...
static size_t get_safe_block_size(size_t dim, size_t point_size_bytes) {
    size_t default_bs = 5000000;
    if (const char* env_p = std::getenv("DISKANN_CHUNK_BYTES")) {
        size_t target_ram = 0;
        try {
            target_ram = std::stoull(env_p);
        } catch (...) {}
        
        if (target_ram > 0) {
            if (point_size_bytes == 0) point_size_bytes = dim * 4; 
            size_t bs = target_ram / point_size_bytes;
            if(bs < 1024) bs = 1024;
            if(bs > 5000000) bs = 5000000;
            return bs;
        }
    }
    return default_bs;
...
static double get_partition_ram_budget_fraction()
{
    constexpr double kDefaultBudgetFraction = 0.95;
    if (const char *env_p = std::getenv("DISKANN_PARTITION_RAM_BUDGET_FRACTION"))
    {
        try
        {
            const double fraction = std::stod(env_p);
    ...
    return kDefaultBudgetFraction;
}

static bool read_actual_shard_counts_from_idmaps(const std::string &prefix_path, int num_parts,
                                                 std::vector<size_t> &shard_counts)
{
    shard_counts.clear();
    shard_counts.reserve(static_cast<size_t>(num_parts));

    for (int p = 0; p < num_parts; p++)
    {
        const std::string idmap_filename = prefix_path + "_subshard-" + std::to_string(p) + "_ids_uint32.bin";
        std::ifstream reader(idmap_filename.c_str(), std::ios::binary);
        if (!reader)
        {
            diskann::cerr << "Error: unable to open shard idmap file for validation: " << idmap_filename << std::endl;
            return false;
        }
        ...
        if (!reader)
        {
            diskann::cerr << "Error: unable to read shard idmap header for validation: " << idmap_filename
                          << std::endl;
            return false;
        }

        shard_counts.push_back(static_cast<size_t>(count));
    }
...
template <typename T>
static bool actual_shards_fit_ram_budget(const std::vector<size_t> &shard_counts, const size_t dim,
                                         const size_t graph_degree, const double ram_budget_gb, const int num_parts,
                                         const size_t attempt, const double budget_fraction)
{
    size_t max_shard_size = 0;
    size_t max_shard_id = 0;
    for (size_t shard = 0; shard < shard_counts.size(); shard++)
    {
        if (shard_counts[shard] > max_shard_size)
        {
            max_shard_size = shard_counts[shard];
            max_shard_id = shard;
        }
    }

    const double max_ram_usage =
        diskann::estimate_ram_usage(max_shard_size, static_cast<uint32_t>(dim), sizeof(T),
                                    static_cast<uint32_t>(graph_degree));
    const double budget_bytes = 1024.0 * 1024.0 * 1024.0 * ram_budget_gb;
    const double effective_budget_bytes = budget_bytes * budget_fraction;

    diskann::cout << "Full partition validation attempt " << attempt << ": P=" << num_parts
                  << ", max actual shard " << max_shard_id << " has " << max_shard_size
                  << " points, estimated RAM usage: " << max_ram_usage / (1024 * 1024 * 1024)
                  << "GB, budget given is " << ram_budget_gb << "GB, acceptance budget is "
                  << effective_budget_bytes / (1024 * 1024 * 1024) << "GB (fraction "
                  << budget_fraction << ")" << std::endl;

    return max_ram_usage <= effective_budget_bytes;
}
                           ...
                           const size_t test_dim, const size_t k_base, std::vector<size_t> &cluster_sizes)
{
    ...
    size_t safe_bs = get_safe_block_size(test_dim, test_dim * sizeof(float));
    size_t block_size = num_test <= safe_bs ? num_test : safe_bs;
                             ...
                             const size_t k_base, std::string prefix_path)
{
    ...
    size_t safe_bs = get_safe_block_size(dim, dim * (sizeof(T) + sizeof(float)));
    size_t block_size = num_points <= safe_bs ? num_points : safe_bs;
...
int retrieve_shard_data_from_ids(const std::string data_file, std::string idmap_filename, std::string data_filename)
{
    ...
    size_t safe_bs = get_safe_block_size(dim, dim * sizeof(T));
    size_t block_size = num_points <= safe_bs ? num_points : safe_bs;
    std::unique_ptr<T[]> block_data_T = std::make_unique<T[]>(block_size * dim);
                              ...
                              size_t graph_degree, const std::string prefix_path, size_t k_base)
{
    ...
    constexpr size_t MAX_FULL_SHARD_VALIDATION_ATTEMPTS = 32;
    size_t full_shard_validation_attempts = 0;
    const double budget_fraction = get_partition_ram_budget_fraction();
    const double effective_ram_budget_bytes = 1024.0 * 1024.0 * 1024.0 * ram_budget * budget_fraction;

    diskann::cout << "Partition RAM budget acceptance fraction: " << budget_fraction
                  << " (effective acceptance budget "
                  << effective_ram_budget_bytes / (1024 * 1024 * 1024) << "GB of " << ram_budget
                  << "GB)" << std::endl;
    ...
    while (!fit_in_ram)
    {
        ...
        diskann::cout << "With " << num_parts
                      ...
                      << "GB, acceptance budget is "
                      << effective_ram_budget_bytes / (1024 * 1024 * 1024) << "GB" << std::endl;
        if (max_ram_usage > effective_ram_budget_bytes)
        {
            fit_in_ram = false;
            num_parts += 2;
            continue;
        ...
        full_shard_validation_attempts++;
        std::vector<size_t> actual_shard_counts;
        if (!read_actual_shard_counts_from_idmaps(prefix_path, num_parts, actual_shard_counts))
        {
            delete[] pivot_data;
            delete[] train_data_float;
            delete[] test_data_float;
        ...
        if (!actual_shards_fit_ram_budget<T>(actual_shard_counts, train_dim, graph_degree, ram_budget, num_parts,
                                             full_shard_validation_attempts, budget_fraction))
        {
            if (full_shard_validation_attempts >= MAX_FULL_SHARD_VALIDATION_ATTEMPTS)
            {
                diskann::cerr << "Error: full partition validation failed after "
                              << full_shard_validation_attempts << " attempts." << std::endl;
                delete[] pivot_data;
                delete[] train_data_float;
                delete[] test_data_float;
                return -1;
            ...
            diskann::cout << "Actual full-data shard exceeds RAM budget. Retrying with " << (num_parts + 2)
                          << " parts." << std::endl;
            fit_in_ram = false;
            num_parts += 2;
        }
