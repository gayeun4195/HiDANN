                                 ...
                                 bool use_opq)
{
    ...
    // Target RAM for buffers: Default 256MB or SIGMA_CHUNK_BYTES env var
    size_t target_ram_bytes = 256 * 1024 * 1024; // 256 MB default

    // Check SIGMA_CHUNK_BYTES first (as used in current experiments), then DISKANN_CHUNK_BYTES
    if (const char* env_p = std::getenv("SIGMA_CHUNK_BYTES")) {
        try { target_ram_bytes = std::stoull(env_p); } catch (...) {}
    } else if (const char* env_p = std::getenv("DISKANN_CHUNK_BYTES")) {
        try { target_ram_bytes = std::stoull(env_p); } catch (...) {}
    }

    // Approximate bytes per point for the 3 heavy buffers + compressed buffer (small)
    size_t bytes_per_point = dim * sizeof(T) + dim * sizeof(float) + dim * sizeof(float); 
    size_t calculated_block_size = target_ram_bytes / bytes_per_point;

    // Clamp block size
    if (calculated_block_size < 1024) calculated_block_size = 1024; 
    if (calculated_block_size > 5000000) calculated_block_size = 5000000;

    size_t block_size = num_points <= calculated_block_size ? num_points : calculated_block_size;
    
    diskann::cout << "Dynamic Block Size for PQ: " << block_size << " (Target Budget: " << target_ram_bytes / (1024*1024) << "MB, Bytes/Pt: " << bytes_per_point << ")" << std::endl;
