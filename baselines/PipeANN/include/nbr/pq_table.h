  ...
  class FixedChunkPQTable {
   public:
    ...
    void load_pq_pivots_new(std::basic_istream<char> &reader, size_t num_chunks, size_t offset) {
      ...
      if (nr != NUM_PQ_OFFSETS && nr != 4) {
        LOG(ERROR) << "Pivot offset incorrect, # offsets = " << nr << ", but expecting " << NUM_PQ_OFFSETS << " or 4";
        crash();
      }
      ...
      if (nr == 5) {
        ...
        // Legacy format (4 offsets): Table -> Centroid -> ChunkOffsets
