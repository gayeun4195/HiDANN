  ...
  class PQNeighbor : public AbstractNeighbor<T> {
   ...
   public:
    ...
    uint64_t query_ctx_size() {
      uint64_t n_chunks = std::max<uint64_t>(this->pq_table.n_chunks, AbstractNeighbor<T>::MAX_BYTES_PER_NBR);
      return 256 * n_chunks * sizeof(float);
    }

    uint64_t neighbor_scratch_bytes() {
      return std::max<uint64_t>(this->pq_table.n_chunks, AbstractNeighbor<T>::MAX_BYTES_PER_NBR);
    }
