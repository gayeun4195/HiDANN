  ...
  void execute_io(void *context, int fd, std::vector<IORequest> &reqs, uint64_t n_retries = 0, bool write = false) {
    ...
    while (true) {
      for (uint64_t j = 0; j < reqs.size(); j++) {
        ...
        if (unlikely(sqe == nullptr)) {
          LOG(ERROR) << "io_uring_get_sqe returned null in execute_io";
          crash();
        }
...
void LinuxAlignedFileReader::register_thread(int flag) {
  if (ioctx::ring == nullptr) {
    ...
    if (unlikely(ret < 0)) {
      LOG(ERROR) << "io_uring_queue_init failed: " << strerror(-ret) << " flag=" << flag;
      delete ioctx::ring;
      ioctx::ring = nullptr;
      crash();
    }
...
void LinuxAlignedFileReader::send_io(IORequest &req, void *ctx, bool write) {
  ...
  auto sqe = io_uring_get_sqe(ring);
  if (unlikely(sqe == nullptr)) {
    LOG(ERROR) << "io_uring_get_sqe returned null in send_io";
    crash();
  }
  req.finished = false;
...
void LinuxAlignedFileReader::send_io(std::vector<IORequest> &reqs, void *ctx, bool write) {
  ...
  for (uint64_t j = 0; j < reqs.size(); j++) {
    auto sqe = io_uring_get_sqe(ring);
    if (unlikely(sqe == nullptr)) {
      LOG(ERROR) << "io_uring_get_sqe returned null in send_io batch";
      crash();
    }
...
void LinuxAlignedFileReader::poll_wait(void *ctx) {
  ...
  if (unlikely(cqe == nullptr)) {
    LOG(ERROR) << "io_uring_wait_cqe returned a null CQE";
    crash();
  }
  if (cqe->res < 0) {
    LOG(ERROR) << "Failed " << strerror(-cqe->res);
...
void LinuxAlignedFileReader::send_io(std::vector<IORequest> &reqs, void *ctx, bool write) {
  ...
  for (uint64_t j = 0; j < n_ops; j++) {
    ...
    cb.data()[j].data = (void *) &reqs[j];  // set user data to point to the request
  }
